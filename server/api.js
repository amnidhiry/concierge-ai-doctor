/**
 * The API, independent of any transport.
 *
 * Extracted from the Vite plugin so the dev server and the production server run
 * the *same* handlers. Two copies of this logic would drift, and the drift would
 * show up as a demo that works on a laptop and fails on the deployed box — which
 * is the worst place to discover it.
 *
 * Every handler is a plain `(req, res, config)` Node HTTP handler. Nothing here
 * imports Vite, and nothing here reads `process.env` directly: configuration is
 * passed in, so the caller owns credential resolution.
 *
 * Routes:
 *   POST /api/intake             — AI-assisted patient intake
 *   POST /api/intake/reset       — clears one client's intake counters
 *   POST /api/care-packet        — source-linked pre-call synthesis
 *   POST /api/visit-documentation — SOAP note, patient summary, billing suggestion
...
const DEFAULTS = { model: DEFAULT_MODEL, maxTokens: DEFAULT_MAX_TOKENS }

/**
 * Normalises whatever the caller resolved into the config every handler expects.
 * Kept here so dev and production cannot disagree about defaults.
 */
export function createApiConfig(env = {}) {
  return {
    anthropic: {
      apiKey: env.anthropic?.apiKey || '',
      model: env.anthropic?.model || DEFAULTS.model,
      maxTokens: Number(env.anthropic?.maxTokens) || DEFAULTS.maxTokens,
...
export function anthropicReady(config) {
  return Boolean(config.anthropic.apiKey)
}

export function livekitReady(config) {
  return Boolean(config.livekit.url && config.livekit.apiKey && config.livekit.apiSecret)
}

/**
 * Startup lines, shared so the dev server and the container report identically.
 * Names the *source* of each credential but never its value — with env vars and
 * .env both in play, "which key is actually in use" is otherwise unanswerable.
 */
export function startupLines(config) {
  const out = []
  if (!anthropicReady(config)) {
    out.push([
      'warn',
      'ANTHROPIC_API_KEY is not set — intake, care packet, and documentation will return a config error.',
    ])
  } else {
    out.push([
      'info',
      `anthropic ready: POST /api/intake, /api/care-packet, /api/visit-documentation (model: ${config.anthropic.model})`,
    ])
    out.push(['info', `anthropic key from: ${config.anthropic.keySource}`])
    out.push([
      'info',
      `intake guards: ${LIMITS.MAX_TURNS} turns/session, ${LIMITS.MAX_PER_WINDOW}/min, ${LIMITS.MAX_CALLS_PER_PROCESS} calls/process`,
    ])
  }
  if (!livekitReady(config)) {
    out.push([
      'warn',
      'LiveKit is not configured — voice visits will show a setup message. Set LIVEKIT_URL, LIVEKIT_API_KEY, LIVEKIT_API_SECRET.',
    ])
  } else {
    out.push(['info', `livekit ready: POST /api/livekit-token (${config.livekit.url})`])
    out.push(['info', `livekit secret from: ${config.livekit.keySource}`])
  }
  return out
}

async function handleIntakeReset(req, res) {
 ...
 *
 * Exact rather than prefix matching, deliberately: the dev server previously had
 * to register `/api/intake/reset` *before* `/api/intake` because Connect matches
 * on prefix and the shorter path would otherwise swallow the reset call. An
 * order-dependent route table is a latent bug, so production matches exactly and
 * order stops mattering.
 */
export const API_ROUTES = new Map([
  ['/api/intake', handleIntake],
  ['/api/intake/reset', handleIntakeReset],
  ['/api/care-packet', handleCarePacket],
  ['/api/visit-documentation', handleVisitDocumentation],
  ['/api/livekit-token', handleLivekitToken],
])

/**
 * Dispatches an API request. Returns false if the path is not an API route, so a
 * caller can fall through to static file serving.
 */
export async function handleApiRequest(req, res, config, pathname) {
  const handler = API_ROUTES.get(pathname)

  if (!handler) {
    // Anything under /api that is not a known route must 404 as JSON. Falling
    // through to the SPA would answer a broken API call with an HTML page, which
    // surfaces as a confusing JSON parse error in the client instead of a 404.
    if (pathname === '/api' || pathname.startsWith('/api/')) {
      sendJson(res, 404, {
        error: { kind: 'not_found', message: `No API route at ${pathname}.` },
      })
      return true
    }
    return false
  ...
  if (req.method !== 'POST') {
    ...
    res.setHeader('Allow', 'POST')
    sendJson(res, 405, {
      error: { kind: 'method_not_allowed', message: 'This route accepts POST.' },
    })
    return true
  }
...
export { sendJson, DEFAULT_MODEL, DEFAULT_MAX_TOKENS }
